...
我們將學習ERC1155標準，它支援一個合約包含多種代幣。

不論是`ERC20`還是`ERC721`標準，每個合約都對應一個獨立的代幣。

假設我們要在以太坊上打造一個類似《魔獸世界》的大型遊戲，這需要我們對每個裝備都部署一個合約。
上千種裝備就要部署和管理上千個合約，這非常麻煩。

因此，以太坊EIP1155提出了一個多代幣標準`ERC1155`，允許一個合約包含`多個同質化`和`非同質化代幣`。ERC1155在GameFi應用程式最多，`Decentraland`、`Sandbox`等知名鏈遊都使用它。

簡單來說
`ERC1155`與`ERC721`類似：

- 在ERC721中，每個代幣都有一個tokenId作為唯一標識，每個tokenId只對應一個代幣；

- 而在ERC1155中，每種代幣都有一個id作為唯一標識，每個id對應一種代幣。
...
```
1. 代幣種類會看到 `mapping(uint256 => mapping(address => uint256)) private _balances;`，這是一個二維陣列，第一維是代幣id，第二維是地址，值是持有的代幣數量。

2. mapping(address => mapping(address => bool)) private _operatorApprovals; 這是一個二維陣列，第一維是擁有者地址，第二維是授權地址，值是是否授權。

3. IERC165 就是確認是否合約實現接口，預防交易到黑洞地址。

4. `balanceOf` 函數是查詢某個地址持有某個代幣的數量。參數有`地址`和`代幣id`。

5. `balanceOfBatch` 多了批量查詢功能，參數有`地址陣列`和`代幣id陣列`。

6. `setApprovalForAll` 函數是授權某個地址可以操作自己的所有代幣。參數有`授權地址`和`是否授權`。

7. `safeTransFerFrom` 安全轉帳基本上和 ERC721 的差不多

8.  一個合約裡面可以定義很多種 token , 可以是代幣，可以是 NFT 都有代號或數量，有點像同捆包的概念，不僅是 id 的分離連 mata data 也是分離的。